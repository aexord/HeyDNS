...
<br>
Далее в зависимости от того, является ли этот хост домен-контроллером или нет, осуществляется попытка переноса зоны (для не-DC DNS-сервер + необходимо предоставить через параметр `--domains` домен/домены) и банальный опрос всех IP входящих в подсеть у DNS-сервера:
<br>
